...
assert.ok(catalog.clients.copilot);
...
assert.equal(catalog.clients.copilot.capabilities.nativePlugin, true);
assert.equal(catalog.clients.copilot.capabilities.cloud, false);
...
assert.match(catalog.dcrSessions, /Copilot/);
...
const copilotConfig = JSON.parse(read('opsphere-copilot/mcp.json'));
const plugin = JSON.parse(read('opsphere-antigravity/plugin.json'));
const copilotPlugin = JSON.parse(read('opsphere-copilot/plugin.json'));
...
assert.equal(copilotConfig.$schema, 'https://agent-plugins.org/schemas/1.0.0/mcp.schema.json');
assert.equal(copilotConfig.mcpServers.opsphere.type, 'streamable-http');
assert.equal(copilotConfig.mcpServers.opsphere.url, catalog.endpoint);
assert.equal(copilotConfig.mcpServers.opsphere.oauth, undefined);
assert.equal(copilotPlugin.$schema, 'https://agent-plugins.org/schemas/1.0.0/plugin.schema.json');
assert.equal(copilotPlugin.name, 'opsphere');
...
for (const file of [
  ...
  'opsphere-copilot/guides/connect.md',
  'opsphere-copilot/skills/connect-another-client/references/connect.md',
]) {
  assert.equal(guide, read(file), `connect guide drift: ${file}`);
}

const sharedBlocks = jsonBlocks(guide);
assert.equal(sharedBlocks.length, 5);
...
assert.deepEqual(sharedBlocks[4], copilotConfig);
...
assert.ok(guide.includes(catalog.copilotPackage.availability));
assert.ok(guide.includes(catalog.antigravityPackage.nativeMcp));
assert.ok(guide.includes(catalog.copilotPackage.nativeMcp));
...
assertClientGuide('opsphere-copilot/guides/copilot.md', copilotConfig, [
  catalog.copilotPackage.availability,
  catalog.copilotPackage.duplicateMcp,
  catalog.copilotPackage.portable,
...
assert.match(read('opsphere-copilot/com.github.copilot/rules/opsphere.md'), /without explicit user consent/);
assert.equal(fs.existsSync('opsphere-antigravity/hooks.json'), false);
assert.equal(fs.existsSync('opsphere-copilot/hooks.json'), false);
assert.equal(fs.existsSync('opsphere-copilot/install.mjs'), false);
assert.equal(fs.existsSync('opsphere-copilot/agents'), false);
assert.equal(JSON.parse(read('.github/plugin/marketplace.json')).plugins[0].source, './opsphere-copilot');
assert.equal(JSON.parse(read('.claude-plugin/marketplace.json')).plugins[0].source, './');
assert.equal(fs.existsSync('marketplace.json'), false);
assert.equal(fs.existsSync('.github/mcp.json'), false);
assert.equal(fs.existsSync('.github/plugin/plugin.json'), false);
...
for (const file of ['plugin.json', 'mcp.json', 'examples/local.md', 'com.github.copilot/agents/endpoint-health.agent.md']) {
  assert.ok(fs.existsSync(`opsphere-copilot/${file}`));
}
