...
Opsphere ships a **Codex plugin** manifest at `.codex-plugin/plugin.json` (version **1.0.16**, separate from Cursor). Both clients use the same gateway: `https://mcp-cursor.opsphere.io/mcp`. Skills under `skills/` are shared (including **`link-account`**, **`open-work-context`**, **`qa-test-investigation`**, and **`qa-release-readiness`**).
