...
description: Get started with Opsphere in OpenCode; discover tools, account and workspace without changing configuration.
---

# Opsphere in OpenCode
...
If disconnected, read the installed `connect-another-client` skill and its reference. Never copy tokens from another app. Installing a skill does not install or authorize MCP. Authenticate with `opencode mcp auth opsphere` or OpenCode `/mcps`.
...
The package supports local OpenCode. Do not copy OAuth files from another client.
