---
description: Recover OAuth when Opsphere disconnects, tools fail with 401, or OpenCode reports invalid_grant.
---

# Opsphere Reconnect

Guide the user through recovering MCP OAuth when tools return 401 or OpenCode reports **`invalid_grant`**. These instructions must remain useful even when MCP tools are unavailable. Do **not** run shell commands on the user's machine.
...
## Re-authenticate in OpenCode

Tell the user:

> 1. Run **`opencode mcp logout opsphere`** then **`opencode mcp auth opsphere`**, or open **`/mcps`**, select **opsphere**, and sign in.
> 2. Complete the browser login with the same account.
> 3. If diagnosis is needed, run **`opencode mcp debug opsphere`** and share only the redacted error class.
> 4. Do not copy token files from another client. OpenCode stores credentials for this app only.
