...
>
> **Needs authentication or OAuth error?** Run **`opencode mcp auth opsphere`** or open `/mcps` in OpenCode and sign in. Do not copy token files from another app.
>
...
1. If MCP is not authenticated, tell the user to run `opencode mcp auth opsphere` or `/mcps`.
