...
Authentication is handled by OpenCode. Do not paste a token.

Tell the user:

> Run **`opencode mcp auth opsphere`** in a terminal, or open **`/mcps`** in OpenCode, select **opsphere**, and sign in. A browser window opens for sign-up or log-in. OpenCode stores the token for this client only.
>
> Use the same email as your other Opsphere clients. Do not copy `mcp-auth.json` or any OAuth files from Cursor, Codex, Claude Code, Warp or Antigravity.
...
If OAuth keeps failing, run **`opencode mcp logout opsphere`** then **`opencode mcp auth opsphere`** once. If the error is `invalid_redirect_uri`, report the callback shape to support; do not change another client's OAuth client ID.
