...
description: Configure an eligible Opsphere provider in the active workspace from GitHub Copilot CLI, respecting plan gates and secure credential entry.
---
...
Use secure credential entry if the client explicitly provides it. If GitHub Copilot CLI has no suitable secret-input surface, guide the user to their authorized configuration surface at https://admin.opsphere.io or Opsphere support. Do not ask for provider secrets in chat, files, shell history, work context or memory; do not assume every Hub member has admin access.
