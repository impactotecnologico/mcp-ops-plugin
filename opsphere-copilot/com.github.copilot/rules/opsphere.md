...
- Opsphere MCP OAuth is supported in local GitHub Copilot CLI. Complete OAuth in this client. Do not copy credentials from Cursor, Codex, Claude Code, Warp, OpenCode or Antigravity.
- Do not add a second Opsphere server named opsphere in `~/.copilot/mcp-config.json` when this plugin is installed. Duplicate entries double the tool catalog.
