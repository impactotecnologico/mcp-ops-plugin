...
- Xray manual tests: when `xray_*` read tools are advertised, use `xray_tests_search` and `xray_test_get` for steps and duplicate detection. Opsphere is not a TMS — do not call `xray_test_create` or `xray_test_steps_update` unless write tools are advertised and the user explicitly confirmed a preview.
