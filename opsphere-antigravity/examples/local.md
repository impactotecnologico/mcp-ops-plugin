...
After installation and independent MCP authorization, ask Antigravity:
...
Complete browser OAuth in Antigravity before these prompts. Do not copy token files from another client. Do not add a second Opsphere server in `~/.gemini/config/mcp_config.json` when this plugin is installed.
