...
- Opsphere MCP OAuth is supported in local Antigravity IDE and CLI. Complete browser OAuth in this client. Do not copy credentials from Cursor, Codex, Claude Code, Warp or OpenCode.
- Do not add a second Opsphere server in `~/.gemini/config/mcp_config.json` or `.agents/mcp_config.json` when this plugin already defines `serverUrl`. Duplicate entries double the tool catalog.
