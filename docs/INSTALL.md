...
Until Opsphere is listed in Cursor's public directory, no clone is required:

1. Open **Customize → Add Marketplace → Import from GitHub**.
2. Enter `https://github.com/opsphere-io/opsphere-plugin` and keep scope **User** (or choose the intended project scope).
3. After import, open the new **Opsphere** marketplace tab and install **opsphere**.
4. Run **Developer: Reload Window**, then use **Connect / Sign in** on the Opsphere plugin card.

The repository uses Cursor's strict multi-plugin layout: `.cursor-plugin/marketplace.json` resolves the installable package from `plugins/opsphere`. The importer tracks `main`; after an update, refresh the imported marketplace before reinstalling or updating the plugin.
...
Opsphere ships a **Codex plugin** manifest at `.codex-plugin/plugin.json` (version **1.0.19**, separate from Cursor). Both clients use the same gateway: `https://mcp-cursor.opsphere.io/mcp`. Skills under `skills/` are shared (including **`link-account`**, **`open-work-context`**, **`qa-test-investigation`**, and **`qa-release-readiness`**).
