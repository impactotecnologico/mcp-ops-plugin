...
Opsphere ships a **Codex plugin** manifest at `.codex-plugin/plugin.json` (version **1.0.9**, separate from Cursor). Both clients use the same gateway: `https://mcp-cursor.opsphere.io/mcp`. Skills under `skills/` are shared (including **`link-account`** and **`open-work-context`** for Connection Hub users).
